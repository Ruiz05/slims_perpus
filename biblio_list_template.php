...
function biblio_list_format($dbs, $biblio_detail, $n, $settings = array(), &$return_back = array()) {
  ...
  $output .= '<div class="item biblioRecord" itemscope itemtype="http://schema.org/Book" vocab="http://schema.org/" typeof="Book"><a href="'.$detail_url.'" class="titleField" itemprop="name" property="name" title="'.__('View record detail description for this title').'"><div class="cover-list">'.$image_cover.'</div></a>';
  ...
  $_check_mark = (utility::isMemberLogin() && $settings['enable_mark'])?'<input type="checkbox" title="'.__('mark this').'" id="biblioCheck'.$_i.'" name="biblio[]" value="'.$biblio_id.'" />':'';
  $output .= '<div class="subItem">'.$detail_button.$xml_button.$cite_button.'</div>';
  ...
  $output .= '<div class="custom-checkbox">'.$_check_mark.'</div>';
