...
else
{
    ...
    // Member name
    createFormContent(__('Member Name dan NIM'), 'text', 'memberName', 'Isikan nama dan NIM anda (exp: Put** dan 1711**)', true, '', true);

    // Birth Date
    createFormContent(__('Birth Date'), 'date', 'memberBirth');

    // Institution
    createFormContent(__('Institution dan Prodi'), 'text', 'memberInst', 'Isikan institusi dan Prodi anda (exp: Univers** dan S1 *****)');

    // Jenis Kelamin
    createSelect(__('Sex'), 'memberSex', [['label' => __('Male'), 'value' => 1],['label' => __('Female'), 'value' => 0]]);

    // Member Address
    createFormContent(__('Address'), 'textarea', 'memberAddress', 'Isikan alamat asal dan sementara anda');

    // Member Mail Address
    createFormContent(__('Phone Number'), 'text', 'memberPhone', 'Isikan Nomor Telepon (whatsapp) anda');

    // Member Mail Address
    createFormContent(__('E-mail'), 'text', 'memberEmail', 'Isikan email UMKT/surel anda');
